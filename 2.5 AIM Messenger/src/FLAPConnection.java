...
/**
 * A FLAPConnection is a thin wrapper around a Socket that handles the FLAP framing protocol that the TOC protocol uses.
 * 
 **/
public class FLAPConnection {
	...
	public void logProtocolMessage(String header, String msg) {
		logger.finer(header + msg);
	}

	public void logOutgoing(String msg) {
		final String header = "C -> S: ";
		logProtocolMessage(header, msg);
	}
	 ...
	 * 
	 * When this method returns, the connection user must send in TOC-level messages. The first message should be the
	 * login messages.
	 * 
	 * @param username
	 *            the username with which to connect. The username must be normalized. @see
	 *            edu.mit.tbp.se.chat.message.MessageLayer#normalizeName
	 * 
	 **/
	synchronized public void connect(String username) throws IOException, SignOnException {
	...
	/**
	 * Disconnect from the server and clean up any resources associated with the connection.
	 * 
	 **/
	 ...
	 * 
	 **/
	synchronized public void writeMessage(String message) throws IOException {
	...
	public String readMessage() throws IOException {
		...
		if (FLAPFrame.KEEP_ALIVE_FRAME_TYPE == frame.frameType) {
			...
			this.logger.warning("readMessage received non-data frame type: " + frame.frameType);

		}

		// otherwise, we have non-ignored data frame, so extract the data
		String payload = new String(frame.data, "US-ASCII");
	 ...
	 * 
	 **/
	synchronized private void writeFLAPSignon(String username) throws IOException {
		// Client To Host:
		...
		System.arraycopy(TLV, 0, payload, VERSION.length, TLV.length);
		packShort((short) username.length(), payload, VERSION.length + TLV.length);
		System.arraycopy(username.getBytes(), 0, payload, VERSION.length + TLV.length + 2 /* size of short */,
				username.length());
	...
	/**
	 * Extract a short from a byte array assumed to be in network byte order (MSB first).
	 * 
	 * @param data
	 *            the bytes to use
	 * @param offset
	 *            the location in the byte array to use.
	 * @return the short represented at data[offset].
	 **/
	private short extractShort(byte[] data, int offset) {
		return (short) (((0xFF & data[offset]) << 8) + ((0xFF & data[offset + 1])));
	}
	 ...
	 * 
	 * @param s
	 *            the short value to pack
	 * @param dest
	 *            the destination byte array
	 * @param offset
	 *            the location to pack
	 **/
	private void packShort(short s, byte[] dest, int offset) {
	...
	private FLAPFrame readFLAPFrame2() throws IOException {
		...
		if (readSize < FLAPFrame.FLAP_HEADER_SIZE) {
			String msg = "Could not read entire FLAP header, read " + readSize + " of " + data.length;
		...
		frame.sequenceNumber = extractShort(data, FLAPFrame.SEQ_OFF);
		short dataLength = extractShort(data, FLAPFrame.LEN_OFF);
		if (dataLength < 0) {
		...
		if (readSize < dataLength) {
			this.logger.warning("Wanted to read " + dataLength + " bytes of frame data, but only read  " + readSize);
		}
	...
	private void fillBuffer(byte[] buff) throws IOException {
	...
	private FLAPFrame readFLAPFrame() throws IOException {
		...
		readDatum = in.read();
		if ('*' != (char) readDatum) {
			this.logger.warning("FLAP frame doesn't start with '*', starts with '" + (int) readDatum + "'");
			// ...but continue if it is not anyway.
		}
		...
		fillBuffer(data);
		frame.sequenceNumber = extractShort(data, 0);
		fillBuffer(data);
		short dataLength = extractShort(data, 0);
		// this.logger.finest("dataLength = " + dataLength);
		if (dataLength < 0) {
			logger.finer("dataLength < 0: [" + Integer.toHexString(data[0]) + "][" + Integer.toHexString(data[1]) + "]");
		}
	...
	private void writeFLAPFrame(FLAPFrame frame) throws IOException {
		...
		packShort(frame.sequenceNumber, shortData, 0);
		out.write(shortData);

		packShort(dataLength, shortData, 0);
		out.write(shortData);
